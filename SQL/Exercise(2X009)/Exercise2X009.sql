...
-- Create a dashboard with the required metrics
SELECT 
    Product_ID,
    SUM(Sales) AS TotalSales,
    SUM(Quantity) AS TotalSalesQuantity,
    COUNT(Order_ID) AS NumberOfOrders,
    MAX(Sales) AS MaximumSalesValue,
    MIN(Sales) AS MinimumSalesValue,
...
-- List of product IDs where the quantity sold exceeds 10
SELECT 
    Product_ID
FROM 
    Sales
